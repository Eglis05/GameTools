...
clan_id = 105 #ZE: 110, RUH: 113, RG:207, IF: 167, SNC: 105
...
def table(initial_rep, total_rep, changes):
    sorted_names = list(dict(sorted(changes[names[3]].items(), key=lambda item: item[1], reverse=True)).keys())
    if sys.platform.startswith('win'):
        os.system('cls')
    else:
        os.system('clear')
    print('|{0:3s} | {1:20s} | {2:10s} | {3:8s} | {4:9s} | {5:12s} | {6:10s} | {7:11s} | {8:11s}'.format("Pos", "Clan Name", "initialRep", "totalRep", "totalGain", names[1], names[2], names[3], names[4]), flush=True)
    line = "-" * 121
    print(line , sep="", end = "\n", flush=True)
